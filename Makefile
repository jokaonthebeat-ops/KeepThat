...
#  The Drum King / MasterGlo Pro / KeyGlo.
#
...
#    make uishot     - render the editor to build/KeepThat-ui.png headlessly
#    make dsptest    - deterministic capture-engine tests
#    make install    - copy bundles into ~/Library/Audio/Plug-Ins
#    make universal  - x86_64 + arm64 fat build
#    make test       - probe the finished VST3 like a DAW would
...
# edits.
PLUGIN_SRCS := $(wildcard $(SRC)/*.cpp) $(wildcard $(SRC)/capture/*.cpp) \
               $(wildcard $(SRC)/dsp/*.cpp) $(wildcard $(SRC)/ui/*.cpp) \
               $(wildcard $(SRC)/state/*.cpp) $(wildcard $(SRC)/export/*.cpp)
...
$(SA_BUNDLE): $(CONFIG_STAMP) $(SA_OBJS) $(SA_OBJ) $(ROOT)/packaging/Standalone-Info.plist \
               $(ROOT)/packaging/KeepThat.icns $(ASSET_FILES)
	...
	@test -f $(ROOT)/packaging/KeepThat.icns \
	  && cp $(ROOT)/packaging/KeepThat.icns $@/Contents/Resources/KeepThat.icns \
	  || true
	$(call COPY_ASSETS,$@)
...
# -----------------------------------------------------------------------------
PKG_DIR      := $(BUILD)/pkg
INSTALLER    := $(DIST)/KeepThat-$(VERSION).pkg
...
installer:
	...
	@# across, and pkgbuild then emits AppleDouble "._KeepThat.vst3" siblings
	...
	@ditto --norsrc --noextattr $(VST3_BUNDLE) "$(PKG_DIR)/root-vst3/Library/Audio/Plug-Ins/VST3/KeepThat.vst3"
	@ditto --norsrc --noextattr $(AU_BUNDLE)   "$(PKG_DIR)/root-au/Library/Audio/Plug-Ins/Components/KeepThat.component"
	@ditto --norsrc --noextattr $(SA_BUNDLE)   "$(PKG_DIR)/root-app/Applications/KeepThat.app"
	...
	@codesign --verify --deep --strict "$(PKG_DIR)/root-vst3/Library/Audio/Plug-Ins/VST3/KeepThat.vst3" \
	  || { echo "  staged VST3 signature is broken"; exit 1; }
	@codesign --verify --deep --strict "$(PKG_DIR)/root-app/Applications/KeepThat.app" \
	  || { echo "  staged app signature is broken"; exit 1; }
	@pkgbuild --root $(PKG_DIR)/root-vst3 --identifier com.diamondloopz.keepthat.vst3 \
	   --version $(VERSION) --install-location / $(PKG_DIR)/KeepThat-VST3.pkg >/dev/null
	@pkgbuild --root $(PKG_DIR)/root-au --identifier com.diamondloopz.keepthat.au \
	   --version $(VERSION) --install-location / $(PKG_DIR)/KeepThat-AU.pkg >/dev/null
	@pkgbuild --root $(PKG_DIR)/root-app --identifier com.diamondloopz.keepthat.app \
	   --version $(VERSION) --install-location / $(PKG_DIR)/KeepThat-App.pkg >/dev/null
	@echo "  VST3, AU and App components built"
	@echo "--- assembling and signing ---"
	@# @VERSION@ is substituted into the distribution and the resources, so a
	@# version bump never leaves stale numbers in the installer's own UI.
	@mkdir -p $(PKG_DIR)/resources
	@sed 's/@VERSION@/$(VERSION)/g' $(ROOT)/packaging/installer/distribution.xml \
	   > $(PKG_DIR)/distribution.xml
	@for f in $(ROOT)/packaging/installer/resources/*; do \
	   sed 's/@VERSION@/$(VERSION)/g' "$$f" > "$(PKG_DIR)/resources/$$(basename $$f)"; \
	 done
	...
	@echo "    xcrun notarytool store-credentials KeepThat --apple-id YOU@EXAMPLE.COM \\"
...
release:
	...
	@xcrun stapler validate "$(INSTALLER)" >/dev/null 2>&1 || { \
	  ...
	  echo "    xcrun notarytool store-credentials KeepThat --apple-id YOU \\"; \
	  echo "      --team-id 922D43C6FJ --password APP-SPECIFIC-PASSWORD"; \
	  echo "    make notarize && make release"; \
	  exit 1; }
	@echo "  installer is notarised and stapled"
	@rm -rf "$(BUILD)/releasezip" && mkdir -p "$(BUILD)/releasezip/KeepThat $(VERSION)"
	@cp "$(INSTALLER)" "$(BUILD)/releasezip/KeepThat $(VERSION)/Install KeepThat.pkg"
	@cp $(ROOT)/packaging/release/ReadMeFirst.txt \
	    "$(BUILD)/releasezip/KeepThat $(VERSION)/Read Me First.txt"
	@cp $(ROOT)/packaging/release/License.txt \
	    "$(BUILD)/releasezip/KeepThat $(VERSION)/License.txt"
	@rm -f "$(BUILD)/KeepThat-$(VERSION)-macOS.zip"
	@cd "$(BUILD)/releasezip" && zip -rXq "$(BUILD)/KeepThat-$(VERSION)-macOS.zip" \
	    "KeepThat $(VERSION)"
	@echo "  $(BUILD)/KeepThat-$(VERSION)-macOS.zip ($$(du -h "$(BUILD)/KeepThat-$(VERSION)-macOS.zip" | cut -f1))"

notarize: $(INSTALLER)
	@echo "Submitting to Apple. This usually takes a few minutes."
	xcrun notarytool submit $(INSTALLER) --keychain-profile KeepThat --wait
...
# -----------------------------------------------------------------------------
clean:
	@rm -rf $(BUILD)/obj $(BUILD)/tools
	@echo "  cleaned objects and tools"
